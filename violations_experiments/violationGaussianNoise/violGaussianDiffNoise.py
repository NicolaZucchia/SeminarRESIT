...
def generate_data_from_dag(dag, n_samples, noise_type="uniform", noise_variances=None):
...
def run_experiment():
    ...
    for run in range(n_runs):
        ...
        # Generate random noise variances
        noise_variances = np.random.uniform(0.1, 1.0, n_variables)
        
        # Uniform noise
        logging.info("\n--- Uniform Noise ---")
        data_uniform = generate_data_from_dag(dag, n_samples, noise_type="uniform", noise_variances=noise_variances)
        ...
        # Gaussian noise
        logging.info("\n--- Gaussian Noise ---")
        data_gaussian = generate_data_from_dag(dag, n_samples, noise_type="gaussian", noise_variances=noise_variances)
