...
def generate_nonlinear_data_from_dag(dag, n_samples):
    """Generate nonlinear data based on a given DAG using sin and Gaussian noise."""
    ...
    for i in range(n_variables):
        ...
        if len(parents) == 0:
            ...
            data[:, i] = np.sin(parent_sum) + np.random.normal(0, 0.1, n_samples)
    
    return pd.DataFrame(data, columns=[f"X{i+1}" for i in range(n_variables)])

def run_single_experiment():
    """Run a single RESIT experiment."""
    n_samples = 500
    n_variables = 3
...
def run_experiment_with_hsic_parallel():
    ...
    n_runs = 100
    results = Parallel(n_jobs=-1)(delayed(run_single_experiment)() for _ in range(n_runs))
